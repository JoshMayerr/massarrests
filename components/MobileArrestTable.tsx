...
interface MobileArrestTableProps {
  arrests?: ArrestLog[];
  total?: number;
  totalPages?: number;
  initialPage?: number;
...
export default function MobileArrestTable({
  arrests: initialArrests,
  total: initialTotal,
  totalPages: initialTotalPages,
  initialPage = 1,
  filters,
}: MobileArrestTableProps) {
  ...
  return (
    <div className="block lg:hidden border-t-2 border-black bg-white">
      {/* Header */}
        ...
        {isLoading ? (
          ...
          <table className="w-full border-collapse">
            <thead className="bg-gray-100 border-b-2 border-black">
              <tr>
                <th className="text-left p-3 text-xs font-extrabold uppercase text-black border-r border-gray-300">
                  Name
                </th>
                <th className="text-left p-3 text-xs font-extrabold uppercase text-black border-r border-gray-300">
                  Info
                ...
                <th className="text-left p-3 text-xs font-extrabold uppercase text-black">
                  Charges
                </th>
              </tr>
            </thead>
              ...
              {arrests.map((arrest, index) => {
                ...
                return (
                  <tr
                    ...
                    className={`bg-white hover:bg-gray-50 transition-colors border-l-4 ${chargeBorderColor} cursor-pointer`}
                  ...
                  >
                    ...
                    <td className="p-3">
                      <div className="flex flex-wrap gap-1">
                        {charges.slice(0, 1).map((charge, i) => (
                          <span
                            key={i}
                            className="px-1.5 py-0.5 text-[10px] bg-black text-white font-bold uppercase border border-black rounded"
                          >
                            {charge.length > 15
                              ? charge.substring(0, 15) + "..."
                              : charge}
                          </span>
                        ))}
                        {charges.length > 1 && (
                          <span className="px-1.5 py-0.5 text-[10px] bg-gray-600 text-white font-bold uppercase border border-black rounded">
                            +{charges.length - 1}
                          </span>
                        )}
                      </div>
      ...
      <ArrestDetailDrawer
        ...
        open={Boolean(selectedArrest)}
        onClose={() => setSelectedArrest(null)}
      />
    </div>
  );
}
